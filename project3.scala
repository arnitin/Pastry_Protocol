...
class PastryNode(/*numRequests :Int , Id :
Int*/InstigatorFellow:Instigator,binarysize:Int) extends Actor {
...
//                                          }
                                          i = returnMatchingBits(curr,binaryfy(x._1))
                                                ...
                                                //println("Didn't " + RoutingTable(0)) // probably here out logic og join
                                                   ...
                                                   conn(connection_list(RoutingTable(0)(i))) ! (x._1,x._2 + 1,"MessagePassing","abcd","abcd",x._6)
                                                }
                                        }
                                  }
                                }//case ending
...
object project3{
  def main(args: Array[String]) : Unit = {
    ...
    if(args.length >1){
      println("Assuming arg0 is numNodes, arg1 is numRequests")
  ...
  //  println("new numNodes is " + numNodes)
   ...
   var NetworkBuilderObj = new NetworkBuilder(numNodes.toInt,numRequests)
...
class Instigator(numPeers:Int,full_buff:ArrayBuffer[Int],conn:ArrayBuffer[PastryNode],numRequests:Int)
extends Actor{
  ...
  var r = new scala.util.Random
    var tempRand = r.nextInt(totNumPeers);
                                ...
                                if(readyPeers == totNumPeers) {
                                        ...
                                        while(j<numRequests){
                                                ...
                                                while(i<totNumPeers){
                                                        ...
                                                        //conn(full_buff(i)) !
(full_buff(tempRand),1,"MessagePassing","MessagePassing",full_buff(i))
                                                        conn(full_buff(i)) !
(full_buff(tempRand),1,"MessagePassing","abcd","abcd",i)
                                                        tempRand = r.nextInt(totNumPeers-1)
                                                        i += 1
                                                }
                                                j +=1
                                        }
                          ...
                          //    println("Instigator received "+x._1 + "  " + x._2 + " "+rxmessages + " " + totalmessages+ " "+ i)
                                ...
                                if(rxmessages == totalmessages){
                                 ...
                                 // println("$$$$$ get the program closed and exit..." + count)
                                  println("Done!!!!Average number of hops for passing "+ numRequests+
" messages for each node is :"+ count/((totNumPeers+1)*numRequests))
                                  j= 0
                                  while(j<totNumPeers){
    ...
    var conn = new ArrayBuffer[PastryNode](numNodes)
     ...
     // val numRequests = 10
      k = 0
       ...
       val instigator = new Instigator(numPeers,full_buff,conn,numRequests)
       instigator.start()

      // initialize all the connections (actors)
      while(k<numNodes){
         var newNode:PastryNode = null
         newNode = new PastryNode(instigator,binarysize)
         newNode.start()
         conn += newNode
         k += 1
      }
